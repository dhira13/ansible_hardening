...
function prep {
  ...
  set -x
  sudo mkdir -p /etc/ansible/roles/thechip911.hardening/
  sudo cp -R ./* /etc/ansible/roles/thechip911.hardening/
  sudo rm /etc/ansible/roles/thechip911.hardening/{*.log,*.html,*.list}
  set +x
...
if [ "$1" == "vagrant" ]; then
  ...
  curl -sSL https://raw.githubusercontent.com/thechip911/ansible-role-hardening/master/defaults/main/suid_sgid_blocklist.yml | grep '  - ' >> "$(date +%y%m%d)-suid.list"
